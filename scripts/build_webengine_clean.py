...
def main() -> int:
    ...
    subprocess.check_call([python, "-m", "pip", "install", "-e", ".[dev]"], cwd=ROOT)
    return subprocess.call([python, "scripts/build_webengine.py"], cwd=ROOT)
...
def _uv_python_install_command(uv_command: Sequence[str]) -> list[str]:
    return [*uv_command, "python", "install", PYTHON_VERSION]


def _uv_venv_command(uv_command: Sequence[str]) -> list[str]:
    return [
        *uv_command, "venv", str(VENV),
