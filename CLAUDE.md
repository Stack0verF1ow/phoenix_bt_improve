...
- `dtorrent_task_v2` uses local fork via `dependency_overrides` in pubspec.yaml (uTorrent peer_id spoof: `-UT3560-`)

### 离线构建流程

CJK 用户名路径（`吴名`）导致 Flutter impellerc / Gradle daemon 崩溃，所以用 robocopy 到 ASCII 路径 `d:\phx-build` 再构建。`PUB_CACHE` 也必须在 D 盘（与构建目录同盘），否则 Kotlin 增量编译器会因跨盘根路径报 `different roots` 错误。
...
# 2. 下载 Dart 依赖
cd D:\phx-build
& "E:\flutter\bin\cache\dart-sdk\bin\dart.exe" "E:\flutter\bin\cache\flutter_tools.snapshot" pub get

# 3. 下载 Gradle 依赖（首次耗时较长）
cd D:\phx-build\android
...
# 或 PowerShell 手动构建：
cd D:\phx-build
& "E:\flutter\bin\cache\dart-sdk\bin\dart.exe" "E:\flutter\bin\cache\flutter_tools.snapshot" build apk --debug
```

**如遇 Kotlin 增量缓存损坏**（`different roots` 错误），清理后重试：

```powershell
Remove-Item -Recurse -Force "D:\phx-build\build", "D:\phx-build\.dart_tool"
```

构建产物：`D:\phx-build\build\app\outputs\flutter-apk\app-debug.apk`
...
4. **CJK 路径**: 用户名 `吴名` 导致 Flutter impellerc / Gradle worker daemon 在 CJK 路径下失败，已在 `build_apk.sh` 中用 robocopy 到 ASCII 路径绕过。`PUB_CACHE` 必须在 D 盘（与构建目录同盘），否则 Kotlin 增量编译报 `different roots` 错误
